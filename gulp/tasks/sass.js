...
const compileFile = function (project) {
  ...
  return (
    gulp
      ...
      .pipe(
        tap(function (file) {
          const str = _.template('Compiling ${src} in ${mode} mode')({
      ...
      .pipe(gulpif(constants.isProdMode, cleanCss()))
      // If the sourcemap flag is passed, write sourcemap to output.
      .pipe(gulpif(util.env['css-sourcemaps'], sourcemaps.write()))
      .pipe(gulp.dest(dest))
      // Log the compilation for this file.
      .pipe(
        tap(function (file) {
          const str = _.template('Compiled to ${dest} in ${mode} mode')({
  ...
  gulp.task('sass', function () {
    ...
    return merge(streams)
      ...
      .on('end', function () {
        ...
        const data = {
          success: (streams.length - errors).toString(),
          count: streams.length.toString(),
          duration: formatted,
        ...
        const elog = _.template(
          '${success} out of ${count} ' + 'sass files compiled without errors'
        )(data);
        if (errors === 0) {
