  ...
  const mergedStream = merge(streams)
    ...
    .on('end', () => {
      ...
      const slog = _.template(
        'Compiled ${count} js files in ${duration} ' + 'with ${errors} errors'
      )(data);

      const elog = _.template(
        '${success} ${"out of"} ${count} ' + 'js files compiled without errors'
      )(data);
...
const appendSourceMaps = () => {
  ...
  _.map(constants.jsSrc, (options) => {
    ...
    logger.log(
      'js',
      'Sourcemap comments appended to compiled js.',
      null,
      false
    );
  });
...
const compilerConfigFactory = (settings) => {
  ...
  if (!settings.filename) {
    throw new Error('No destination filename for closure compiler.');
  }

  if (!settings.entryPoint) {
    throw new Error('No entry point for closure compiler.');
  }
  ...
  return {
    ...
    compilerFlags: {
      ...
      // TODO: re-enable conformance chacks when a TS lib is avaiable to mitigate these violations.
      //conformance_configs: path.join(constants.paths.CLOSURE,'conformance_proto.txt'),
...
const compileSingleConfig = (options) => {
  ...
  const tsStream = gulp
    ...
    .on('end', function () {
      ...
      const log = `Compiling ${data.entry} from ${data.count} files to
          ${data.dest} (${data.deps} deps, ${data.source} source)`;
      logger.log('js', log);
      start = moment();
    })
    .pipe(
      tsProject().on('error', function (err) {
        options.error = true;
    ...
    .on('end', function () {
      ...
      logger.log(
        'js',
        `Compiled ${options.entry.toString()} module in ${formatted}`,
        null,
        false
      );
      start = moment();
  ...
  const closureStream = gulp.src(constants.closureLib, { base: './' });

  return (
    merge(closureStream, tsStream)
      .pipe(
        closureCompiler(compilerConfig.compilerFlags).on(
          'error',
          function (err) {
            options.error = true;
            logger.error('js', options.dist, err.message, true);
      ...
      .on('end', function () {
        ...
        logger.log(
          'js',
          `Compiled ${options.entry.toString()} js module in ${formatted}`,
          null,
          false
        );
      })
      .pipe(
        size({
          ...
          log: (title, what, size) => {
            ...
            logger.log(
              'size',
              `gZipped size of ${what} is ${sizeKb} Kb`,
              null,
              false
            );
          },
